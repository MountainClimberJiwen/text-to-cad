...
from cad_asm.commands import init, step, verify, export, lib, run
...
def main(argv: list[str] | None = None) -> int:
    ...
    # run
    p_run = sub.add_parser("run", help="Auto-run assembly loop until complete")
    p_run.add_argument("--workspace", "-w", type=Path, default=Path("."), help="Workspace directory")
    p_run.add_argument("--auto", action="store_true", help="Auto-approve review gates and continue")
    p_run.add_argument("--max-iterations", type=int, default=100, help="Safety cap to prevent infinite loops")
    p_run.add_argument("--continue", dest="continue_", action="store_true", help="Continue from in_review state (deprecated, use --auto)")
    ...
    if args.command == "run":
        return run.run(args.workspace, auto=args.auto or args.continue_, max_iterations=args.max_iterations)
